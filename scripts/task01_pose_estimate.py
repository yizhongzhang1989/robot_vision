#!/usr/bin/env python3
"""
3D Positioning Estimation using Triangulation
...
class KeypointMatcher:
    """Class to handle keypoint detection and matching using FlowFormer optical flow"""
    
    def __init__(self, camera_params: CameraParameters, data_dir: str = "test_data", server_url="http://msraig-ubuntu-3:5000"):
        """
        ...
        Args:
            ...
            server_url: FlowFormer server URL
        """
        self.camera_params = camera_params
        self.data_dir = Path(data_dir)
        self.tracker = KeypointTracker(server_url=server_url)
        logger.info(f"Initialized FlowFormer client with server: {server_url}")
    
    def track_keypoints_with_optical_flow(self, ref_image: np.ndarray, target_image: np.ndarray,
                                        ref_points: List[Dict], save_visualization: bool = True, sequence_idx: int = 0) -> Tuple[np.ndarray, np.ndarray]:
        """
        Track keypoints using FlowFormer optical flow
        ...
        if self.tracker is None:
            raise RuntimeError("FlowFormer tracker is not initialized. Cannot perform keypoint tracking.")
        
        # Convert reference points to the format expected by KeypointTracker
        tracker_keypoints = []
        for pt in ref_points:
            tracker_kp = {
                'id': pt.get('id', len(tracker_keypoints) + 1),
                'name': pt.get('name', f'Point {pt.get("id", len(tracker_keypoints) + 1)}'),
                'x': pt['x'],
                'y': pt['y'],
        ...
        # Compute optical flow using FlowFormer
        flow = self.tracker.client.compute_flow(ref_image, target_image)
        
        # Track keypoints using flow
        tracked_keypoints = self.tracker.track_keypoints_with_flow(tracker_keypoints, flow)
        
        # Extract tracked coordinates
        target_pts = np.array([[kp['new_x'], kp['new_y']] for kp in tracked_keypoints], dtype=np.float32)
        
        logger.info(f"Successfully tracked {len(target_pts)} keypoints using FlowFormer")
    ...
    def visualize_tracking_results(self, ref_image: np.ndarray, target_image: np.ndarray,
        ...
        # Save visualization
        output_path = self.data_dir / "positioning_data" / filename
...
class PositioningEstimator:
    """Main class for 3D positioning estimation"""
    ...
    def __init__(self, data_dir: str = "test_data"):
        ...
        self.keypoint_matcher = KeypointMatcher(self.camera_params, str(self.data_dir))
    ...
    def estimate_3d_positions(self, sequence_indices: Optional[List[int]] = None) -> Dict:
        """
        Perform complete 3D position estimation
        ...
        logger.info("Starting 3D position estimation...")
    ...
    def save_results(self, results: Dict, output_file: str = "positioning_results.json"):
        """
        ...
        # Print summary
        print("\n" + "="*60)
        print("3D POSITIONING ESTIMATION RESULTS")
        print("="*60)
...
def main():
    """Main function to run 3D positioning estimation"""
    try:
        ...
        # Save results
        estimator.save_results(results, "3d_positioning_results.json")
        
        print("3D positioning estimation completed successfully!")
